...
func _on_owner_exited() -> void:
    if source == null or !source.has_signal(signal_name) or !source.get(signal_name).is_connected(_on_event_triggered):
        return
    
    source.get(signal_name).disconnect(_on_event_triggered)
